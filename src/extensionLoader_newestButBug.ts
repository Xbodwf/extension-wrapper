...
export class ExtensionLoader extends Array<any> {
  ...
  loadExtensionFromModules(modules: Record<string, any>) {
      ...
      if (typeof fn === "function") {
        ...
        if (fnStr.includes("return N.e(")) {
          ...
          if (match) {
            ...
            console.log(`[Wrapper Main] Executing extension loader checked \n chunkId=${chunkId}, moduleId=${moduleId}`);
          }
        }
      }
    });
    ...
    regularCandidateFns.forEach((moduleFn) => {
      try {
        ...
        const decompiledResult = this.decompile(fnStr);
        let moduleResult = this.require(undefined, moduleFn, modules);
        results.push(moduleResult);
      } catch (e) {
        console.warn("[Wrapper Main] Failed to process module or load module:", e);
      }
    });
      ...
      if (detectedLoaderChunks.has(parseInt(chunkId))) {
        try {
          if (this.moduleRegistry && this.moduleRegistry[moduleId]) {
            ...
            if (moduleResult) {
              results.push(moduleResult);
              console.log(`[Wrapper Main] Loaded chunkId=${chunkId} moduleId=${moduleId}`);
            }
          }
        } catch (e) {
          console.warn(`[Wrapper Main] Failed to load chunkId=${chunkId} moduleId=${moduleId}:`, e);
        }
      }
    }
      */
    ...
    if (results.length > 0) {
      try {
        ...
        if (moduleExports.__esModule === true) {
          target = this.getClass(moduleExports.default,Scratch?.vm);
        } else {
          target = this.getClass(moduleExports,Scratch?.vm);
        }
        if(target === null){
          return;
        }
        ...
        console.log("[Wrapper Main] Successfully registered extension:", info.id);
      } catch (e) {
        console.warn("[Wrapper Main] First Method failed...", e);
        try {
          ...
          console.warn("[Wrapper Main] Failed to register extensions.", e);
        }
      }
    }
  }
  ...
  getClass(classFunc: Function,...args): any {
    return classFunc?.prototype?.constructor == classFunc ? new (classFunc as any)(...args) : null;
  }
